...
async function main() {
  ...
  try {
    ...
    assert.equal(migration.version, '3.6.4');
    ...
    assert.ok(caches.includes('treino-hard-v3.6.4'));
    ...
    console.log(JSON.stringify({url: baseUrl, migration, caches, offline: true, errors, screenshots: fs.readdirSync(OUTPUT).map(file => path.join(OUTPUT, file))}, null, 2));
  } finally {
    await browser.close();
